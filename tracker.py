...
try:
    ...
    print(f"Error reading Google Sheet. Ensure it is shared as 'Anyone with link can view'. Details: {e}")
    exit(1)

# 2. Configure the Gemini AI Engine
...
for index, row in df.iterrows():
    ...
    # Generate an encoded query for Google News
    ...
    # 3. Prompting Gemini to synthesize both sources concisely
    prompt = f"""
    ...
    try:
        ...
        # Only add to the report if the AI actually found something new
...
# 4. Ship it to Telegram if there's actionable news
if updates_found:
